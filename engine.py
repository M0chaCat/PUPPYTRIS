...
BOARD_HEIGHT_PX = settings.CELL_SIZE * (
    settings.BOARD_HEIGHT - settings.BOARD_EXTRA_HEIGHT
)

BOARD_PX_OFFSET_X = (settings.WINDOW_WIDTH - BOARD_WIDTH_PX) / 2
BOARD_PX_OFFSET_Y = (
    settings.WINDOW_HEIGHT - BOARD_HEIGHT_PX - (settings.WINDOW_HEIGHT * 0.05)
) / 2 - (settings.BOARD_EXTRA_HEIGHT * settings.CELL_SIZE)
...
def update_starting_coords():
    ...
    starting_x = (
        settings.BOARD_WIDTH - piece_width
    ) // 2  # dynamically calculate starting position based on board and piece size.
...
def spawn_piece():
    ...
    piece_board = (
        pieces_dict[piece_bags[0][0]]["shapes"][piece_rotation] * piece_bags[0][0]
    )
    gen_next_boards()
    gen_topout_board()
    update_history()
...
def undo(amount):
    ...
    if pieces_placed - amount >= 0:
        ...
        if history_index < 0:
            history_index = (
                settings.MAX_HISTORY - amount
            )  # make sure this doesn't miss one
...
def gen_topout_board():
    global topout_board
    next_width = pieces_dict[(piece_bags[0] + piece_bags[1])[1]]["shapes"][
        piece_rotation
    ].shape[0]
    board_size = next_width
    topout_board = numpy.zeros((board_size, board_size), dtype=numpy.int8)
    next_shape = pieces_dict[(piece_bags[0] + piece_bags[1])[1]]["shapes"][
        STARTING_ROTATION
    ]

    # --- Check top 4/5 rows for occupancy ---
    top_rows = (
        settings.BOARD_EXTRA_HEIGHT
        + next_width
        + math.floor(settings.BOARD_HEIGHT / 8)
        - 1
    )  # 1 row less for boards < 24 high, and 2 less for boards < 8 high
...
def update_ghost_piece():  # make sure piece_board has been updated before calling this function
    ...
    for _ in range(piece_y, settings.BOARD_HEIGHT):
        if not check_collisions(
            0, 1, piece_board, ghost_piece=True
        ):  # fourth param for ghost pieces
            ghost_piece_y += 1
        else:
            break
    # update the board
...
def check_collisions(target_move_x, target_move_y, target_shape, ghost_piece=False):
    ...
    for coords in numpy.argwhere(
        target_shape != 0
    ):  # returns a 1d numpy array of coordinates that meet the condition != 0
        if (
            coords[0] + new_y > settings.BOARD_HEIGHT - 1
        ):  # check for collision with the bottom of the board
            collided = True
            break
        if (
            coords[1] + new_x < 0 or coords[1] + new_x > settings.BOARD_WIDTH - 1
        ):  # check for collision with the sides of the board
            collided = True
            break
        if game_board[
            coords[0] + new_y, coords[1] + new_x
        ]:  # check for collision with minos
            collided = True
            break
    if collided:
        return True
...
def check_touching_ground():
    ...
    for coords in numpy.argwhere(piece_shape != 0):
        if (
            coords[0] + piece_y + 1 > settings.BOARD_HEIGHT - 1
        ):  # check if its touching the bottom of the board
            return True  # return true if it IS touching the ground
        elif game_board[
            coords[0] + piece_y + 1, piece_x + coords[1]
        ]:  # check if its touching any minos below it
            return True
...
def rotate_piece(amount):
    ...
    #                                                                                  ---
    if new_rotation == 0 and piece_bags[0][0] in (
        1,
        3,
        4,
        5,
    ):  # ensures the kick order is symmetrical for Z, S, O, I
    ...
    for kick in kick_list:
        kick_x, kick_y = kick
        if not check_collisions(
            kick_x, kick_y, new_shape
        ):  # continue if no collisions found
...
def hold_piece():
    ...
    if len(hold_pieces) >= hold_pieces_count:  # if hold bag is full
        new_shape = pieces_dict[hold_pieces[0]]["shapes"][
            piece_rotation
        ]  # returns the next piece in the hold queue
    else:
        new_shape = pieces_dict[(piece_bags[0] + piece_bags[1])[1]]["shapes"][
            piece_rotation
        ]  # returns the next piece in the next queue

    if not check_collisions(0, 0, new_shape):
        hold_pieces.append(
            piece_bags[0][0]
        )  # take the current piece and add it to hold queue
        ...
        if len(hold_pieces) > hold_pieces_count:
            piece_bags[0].insert(
                0, hold_pieces[0]
            )  # insert the next hold piece as the new current piece
    ...
    piece_board = (
        pieces_dict[(piece_bags[0] + piece_bags[1])[0]]["shapes"][piece_rotation]
        * piece_bags[0][0]
    )  # gets the next piece, this implementation is required cause holding can sometimes empty bag 1
    update_ghost_piece()
    gen_hold_boards()
...
def hold_guideline(infinite_holds=False):
    ...
    if holds_left > 0 or infinite_holds:
        ...
        # refresh current active piece
        piece_board = (
            pieces_dict[(piece_bags[0] + piece_bags[1])[0]]["shapes"][STARTING_ROTATION]
            * piece_bags[0][0]
        )  # gets the next piece, this implementation is required cause holding can sometimes empty bag 1
        update_starting_coords()
        piece_rotation = STARTING_ROTATION
...
def move_piece(move_x, move_y):
    ...
    move_dir_x = int(
        (move_x > 0) - (move_x < 0)
    )  # treats bools like integers then converts to int to get direction
    ...
    for step in range(
        steps_to_move
    ):  # loops over whichever number is farther from 0 (the most moves), min 1
        if not check_collisions(
            move_dir_x, move_dir_y, current_shape
        ):  # only goes through with the movement if no collisions occur
            piece_x = (
                move_dir_x + piece_x
            )  # int(move_x > 0) returns 0 if move_x is 0, and 1 otherwise
...
def gen_next_boards():
    ...
    next_boards = []
    next_list = (piece_bags[0] + piece_bags[1])[
        1 : settings.NEXT_PIECES_COUNT + 1
    ]  # gets a truncated next_pieces list
...
def clear_lines():
    ...
    if line_clear_count > 0:
        ...
        board_mask = game_board[
            ~lines_to_clear
        ]  # masks the board, removing lines where the mask returned true
        new_lines = numpy.zeros(
            (line_clear_count, settings.BOARD_WIDTH), dtype=numpy.int8
        )
...
def lockdown_classic(frametime):  # NEED TO IMPLEMENT PIECE FLASHING. entry reset style.
    global \
        lockdown_timer, \
        prevent_harddrop_timer, \
        prevent_harddrop_clock, \
        prevent_harddrop_timer_started
...
def lockdown_step(
    frametime,
):  # NEED TO IMPLEMENT PIECE FLASHING. step reset (TGM) style.
    global \
        lockdown_timer, \
        prevent_harddrop_timer, \
        prevent_harddrop_clock, \
    ...
    if lockdown_timer == 0:
        lockdown_start_y = piece_y  # set the initial variable
    lockdown_timer += frametime
...
def lockdown_guideline(
    frametime,
):  # NEED TO IMPLEMENT PIECE FLASHING. move reset (guideline) style.
    global \
        lockdown_timer, \
        prevent_harddrop_timer, \
        prevent_harddrop_clock, \
    ...
    global \
        lockdown_start_x, \
        lockdown_start_y, \
        lockdown_start_rotation, \
        lockdown_resets_left
    ...
    piece_moved = (
        ...
        or lockdown_start_y != piece_y
        or lockdown_start_rotation != piece_rotation
    )
...
def lock_piece():
    ...
    for coords in numpy.argwhere(piece_board != 0):
        new_board[piece_y + coords[0], piece_x + coords[1]] = piece_board[
            coords[0], coords[1]
        ]
...
def handle_movement(keys):
    ...
    if move_dir != 0:
        if (
            last_move_dir != move_dir and settings.DAS_RESET_THRESHOLD <= 0
        ):  # if switching movement direction (and DAS_RESET_THRESHOLD is set to 0), reset DAS and ARR
        ...
        das_timer_started
    ):  # saves performance by only checking this stuff when das_timer is still running
        ...
        if (
            das_reset_timer >= settings.DAS_RESET_THRESHOLD
        ):  # if das reset timer goes through, then reset all timers
            das_timer = 0
            das_timer_started = False
            das_reset_timer = 0
            das_reset_timer_started = False
...
def unpack_1kf_binds():
    ...
    for row, col in numpy.ndindex(
        (4, 10)
    ):  # indexes through all coordinates in a 4x10 array
        string_index = (row * 10) + col
        onekf_key_array[row][col] = pygame.key.key_code(
            settings.ONEKF_STRING[string_index : string_index + 1]
        )
...
def handle_1kf(key, keydir="DOWN"):
    ...
    current_shape = pieces_dict[piece_bags[0][0]]["shapes"][piece_rotation]
    rightmost_point = numpy.max(
        numpy.where(current_shape != 0)[1]
    )  # gets the distance between piece_x and the rightmost point of the piece
    leftmost_point = numpy.min(
        numpy.where(current_shape != 0)[1]
    )  # this is usually 0, but needed for O piece

    if key_col < 5:
        steps_to_move = int(
            key_col - piece_x - leftmost_point
        )  # cast to int since numpy.where always returns an array for some reason
    else:
        steps_to_move = int(
            key_col - piece_x - rightmost_point
        )  # cast to int since numpy.where always returns an array for some reason

    game_state_changed = True
    move_piece(
        steps_to_move, 0
    )  # done in two steps because otherwise it stops early when colliding with the wall
...
def reset_game():
    ...
    das_timer_started = arr_timer_started = sdr_timer_started = (
        das_reset_timer_started
    ) = prevent_harddrop_timer_started = False
...
def handle_soft_drop(keys, frametime):
    ...
    if current_gravity > 0.001:
        softdrop_overrides = (
            settings.SDR_THRESHOLD <= 16.666667 / current_gravity
            and keys[settings.MOVE_SOFTDROP]
        )  # returns true if softdrop is pressed and is faster than gravity
    elif keys[settings.MOVE_SOFTDROP]:
        softdrop_overrides = (
            True  # returns true always if gravity is 0 (prevents divide by 0)
        )
    else:
    ...
    if settings.SDR_THRESHOLD == 0:
        ...
        steps_to_move = max(
            int(frametime / settings.SDR_THRESHOLD), 1
        )  # predicts how much softdrop should move for first button press
...
def hard_drop():
    ...
    if (
        not prevent_harddrop_timer_started
        or prevent_harddrop_timer >= settings.PREVENT_HARDDROP_THRESHOLD
    ):
...
def do_gravity(frametime):
    ...
    if current_gravity >= 19.8:  # make instant drop at 20g regardless of framerate
        ...
        not softdrop_overrides
    ):  # only process gravity this frame if user isn't pressing the softdrop key
...
def do_leftover_gravity(
    remaining_steps,
):  # for when a piece falls, touches the ground, then loses ground inside the same frame. prevents hanging for a frame.
    if remaining_steps != 0:  # check if the piece can move at all
        move_piece(
            0, remaining_steps
        )  # move the piece by the leftover amount from this frame
...
def swap_mode(usepenta):
    ...
    PIECE_TYPES = (
        settings.PIECE_TYPES_PENTA if settings.is_penta else settings.PIECE_TYPES_TETRA
    )
    pieces_dict = pieces.tetra_dict
    piece_inversions = pieces.tetra_inversions
    hold_pieces_count = settings.HOLD_PIECES_COUNT_TETRA
