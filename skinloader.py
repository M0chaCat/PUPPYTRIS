...
sprite_file = next(
    file for file in os.listdir(skins_dir) if file.lower().endswith(".png")
)
...
for i in range(settings.PIECE_TYPES_TETRA):
    ...
    scaled_skin = pygame.transform.scale(
        raw_skin, (settings.CELL_SIZE, settings.CELL_SIZE)
    )
...
def set_other_skins():
    for i in range(5):
        cell_x = (cell_size_tetra + cell_spacing) * (
            settings.PIECE_TYPES_TETRA + i
        )  # skip tetraminos
        cell_rect = pygame.Rect((cell_x, 0), (cell_size_tetra, cell_size_tetra))
...
settings.WALLPAPER_PATH = (
    os.path.join(wallpaper_dir, wallpaper_file) if wallpaper_file else None
)
